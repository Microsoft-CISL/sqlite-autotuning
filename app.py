...
# # Streamlit Configuration
# # --------------------------------------------------------------------------------
# st.set_page_config(page_title="Azure MySQL Config Analyzer", layout="wide")
...
# # --------------------------------------------------------------------------------
# st.sidebar.title("Azure MySQL Config Analyzer")
# experiment_id = st.sidebar.selectbox("Select Experiment", options=list(storage.experiments.keys()))
...
#         "Clustering",
#         "Save Analysis",
#         "Config Params Scatter",  # New Tab Added Here
#     ]
# )
...
# # --------------------------------------------------------------------------------
# # TAB 2: Configurations Analysis
# # --------------------------------------------------------------------------------
# with tab_config_analysis:
#     st.header("Configurations Analysis")
#     config_id = st.selectbox(
#         "Select Configuration ID", df["tunable_config_id"].unique(), key="config_id"
#     )
#     metric = st.selectbox("Select Metric", metrics, key="config_metric")
...
#         x="trial_id",
#         y=metric,
#         title=f"{metric} over Trials for Configuration {config_id}",
#         markers=True,
#         labels={"trial_id": "Trial ID", metric: metric},
#     )
#     st.plotly_chart(fig, use_container_width=True)
...
#     st.header("Optimization Suggestions")
#     target_metric = st.selectbox("Select Metric for Optimization", metrics, key="target_metric")
#     optimization_method = st.radio(
#         "Select Optimization Method", ["Maximize", "Minimize"], key="opt_method"
#     )
...
#         st.write(f"**Optimal Configuration ({optimization_method} {target_metric})**:")
#         st.json(optimal_config[config_columns].to_dict())
#     else:
#         st.warning("No data available for optimization.")

# # --------------------------------------------------------------------------------
# # TAB 4: Interactive Correlation Heatmap
# # --------------------------------------------------------------------------------
# with tab_heatmap:
#     st.header("Correlation Heatmap")
#     selected_columns = st.multiselect(
#         "Select Columns for Heatmap", config_columns + result_columns, key="heatmap_cols"
#     )

#     if st.button("Generate Heatmap", key="heatmap_button") and selected_columns:
#         corr_matrix = df[selected_columns].corr()
#         fig = px.imshow(
#             corr_matrix,
#             text_auto=True,
...
#         failure_rate_trend = (
#             df.groupby("trial_id")["status"].apply(lambda x: (x == "FAILED").mean()).reset_index()
#         )
#         failure_rate_trend.columns = ["Trial ID", "Failure Rate"]
#         fig = px.line(
#             failure_rate_trend,
...
# # --------------------------------------------------------------------------------
# # TAB 6: Top and Bottom Configurations
# # --------------------------------------------------------------------------------
# with tab_top_bottom:
#     st.header("Top and Bottom Configurations")
#     n_configs = st.slider("Number of Configurations to Display", 1, 10, 5, key="n_configs_display")

#     # We use the same target metric from the optimization tab, or fallback
#     tb_metric = target_metric if "target_metric" in st.session_state else metrics[0]
#     st.write(f"Using metric: **{tb_metric}**")
...
#         st.subheader("Top Configurations")
#         st.write(top_configs)

#         st.subheader("Bottom Configurations")
#         st.write(bottom_configs)
#     else:
#         st.warning("No data available to identify top/bottom configurations.")

# # --------------------------------------------------------------------------------
# # TAB 7: Parallel Coordinates Plot
# # --------------------------------------------------------------------------------
# with tab_parallel_coords:
#     st.header("Parallel Coordinates Plot")
#     parallel_columns = st.multiselect(
#         "Select Columns for Parallel Plot", config_columns + result_columns, key="parallel_cols"
#     )

#     # We use the same target metric from earlier or fallback
#     pc_metric = (
#         target_metric if "target_metric" in st.session_state else (metrics[0] if metrics else None)
#     )

#     if (
#         st.button("Generate Parallel Plot", key="parallel_button")
#         and parallel_columns
#         and pc_metric
#     ):
#         fig = px.parallel_coordinates(
#             df,
...
#             color=pc_metric,
#             title="Parallel Coordinates Plot",
#             labels={col: col for col in parallel_columns},
#         )
#         st.plotly_chart(fig, use_container_width=True)
#     else:
#         st.info("Select columns and metric to generate the parallel coordinates plot.")

# # --------------------------------------------------------------------------------
# # TAB 8: Configuration Clustering
# # --------------------------------------------------------------------------------
# with tab_clustering:
#     st.header("3D Scatter Plot with Clustering")

#     cluster_columns = st.multiselect(
#         "Select Columns for Clustering", config_columns + result_columns, key="cluster_cols"
#     )
#     num_clusters = st.slider("Number of Clusters", 2, 10, 3, key="cluster_slider")

#     if len(cluster_columns) >= 2:
#         if st.button("Generate Clustering", key="clustering_button"):
#             clustering_data = df[cluster_columns].dropna()
#             kmeans = KMeans(n_clusters=num_clusters, random_state=42)
#             kmeans.fit(clustering_data)
#             df["cluster"] = kmeans.labels_

#             # If only 2 columns are selected, we duplicate the second column for 'z'
#             x_col, y_col = cluster_columns[0], cluster_columns[1]
#             z_col = cluster_columns[2] if len(cluster_columns) > 2 else cluster_columns[1]
...
#         )
# # -------------------------------------------------------------------------------
# # TAB 10: Configuration Parameters Scatter Plot
# # --------------------------------------------------------------------------------
# with tab_config_scatter:
#     st.header("Configuration Parameters Scatter Plot")
#     st.write(
#         "Analyze the relationship between multiple configuration parameters and a selected result metric."
#     )

#     # Ensure there are configuration and result columns available
#     if not config_columns:
#         st.warning("No configuration parameters available in the dataset.")
#     elif not metrics:
#         st.warning("No result metrics available in the dataset.")
...
#             "Select Configuration Parameters",
#             options=config_columns,
#             default=config_columns[:2],  # Default to first two for convenience
#             help="Choose one or more configuration parameters to analyze.",
#         )
...
#         if selected_config_params:
#             # Determine the number of columns for layout (e.g., 3 plots per row)
#             plots_per_row = 3
#             num_plots = len(selected_config_params)
#             num_rows = (num_plots + plots_per_row - 1) // plots_per_row
...
#                                 },
#                                 hover_data=["trial_id", "tunable_config_id"],
#                                 trendline="ols",  # Adds a trendline for better insight
#                                 template="plotly_white",
#                             )
...
# st.sidebar.markdown(
#     """
# - **Use the Data Overview tab** first to understand your dataset.
# - **Configurations Analysis** helps visualize trials for a specific config.
# - **Optimization Suggestions** highlights potential best/worst configs.
# - **Use the Heatmap, Failure Analysis, and Parallel Coordinates** to deep-dive into correlations and distribution.
# - **Try Clustering** to discover hidden grouping in your data.
# - Finally, **save your analysis** via the Download Report tab.
# """
# )
